...
Finally, introduce the region and the output format to be used:

```{r aws-sso-client, fig.cap = "Introduce the CLI region and output format.", echo = FALSE}
knitr::include_graphics('figures/04-AWS/aws_sso_client.PNG')
```

If everything has gone as expected, you should now be able to list our set of buckets that we have on AWS (your profile name should have been given as part of the previous command output):
